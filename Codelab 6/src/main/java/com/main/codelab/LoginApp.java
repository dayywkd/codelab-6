...
public class LoginApp extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        Runnable loginAction = () -> {
            if ("Udaywakid".equals(userTextField.getText()) && "7890".equals(pwBox.getText())) {
    ...
    private void showWelcomePage(Stage stage) {
        ...
        Text welcomeText = new Text("Hello Udaywakid!\nWelcome to Codelab!!");
        welcomeText.setFont(Font.font("Tahoma", FontWeight.NORMAL, 20));
        welcomeGrid.add(welcomeText, 0, 0, 2, 1);

        Button btnl = new Button("Back");
        HBox hbBtnl = new HBox(10);
        hbBtnl.setAlignment(Pos.BOTTOM_CENTER);
        hbBtnl.getChildren().add(btnl);
        welcomeGrid.add(hbBtnl, 1, 1);
